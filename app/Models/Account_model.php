    ...
    //authenticate user email and password
     public function authenticateUser($email,$password){
       ...
       $user = $query->getRow(0);
        ...
        if($user){
          if($user->thumbnail!=""){
            $user->thumbnail = base_url()."/uploads/members/".$user->thumbnail;
          }
          if($user->coverphoto!=""){
            $user->coverphoto = base_url()."/uploads/members/".$user->coverphoto;
          }
        }
        return $user;
    }
     ...
     public function createAccount($email,$password){
       ...
       $user = $query->getRow(0);
        if($user){
          ...
          $info = array('email' => $email,'password' => password_hash($password, PASSWORD_DEFAULT));
          $builder->insert($info);
          $this->status = "ok";
        }
    }
