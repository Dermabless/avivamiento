...
class Socials_model extends Model
{
  ...
  public function __construct()
    ...
    function getuserBioInfo($email){
      ...
      if($user){
         $user->activated = 1;
         $user->photo = $user->thumbnail==""?"":base_url()."/uploads/members/".$user->thumbnail;
         $user->coverphoto = $user->coverphoto==""?"":base_url()."/uploads/members/".$user->coverphoto;
         //$res->following = 1;
        return $user;
      }else{
        return null;
    ...
    function usersToFollowListing($page = 0, $query = "", $email=""){
      ...
      foreach ($result as $res) {
        $res->photo = $res->thumbnail==""?"":base_url()."/uploads/members/".$res->thumbnail;
        $res->coverphoto = $res->coverphoto==""?"":base_url()."/uploads/members/".$res->coverphoto;
        $res->following = 1;
      }
      return $result;
    }
    ...
    public function fetch_posts($page = 0,$email=""){
      ...
      foreach ($result as $res){
        $res->photo = base_url()."/uploads/members/".$res->thumbnail;
        $res->coverphoto = $res->coverphoto==""?"":base_url()."/uploads/members/".$res->coverphoto;
    ...
    /*public function get_total_comments($id){
     ...
     function getUpdatedUserProfile($email){
       ...
       if($user){
          ...
          $user->photo = $user->thumbnail==""?"":base_url()."/uploads/socials/avatars/".$user->thumbnail;
          $user->coverphoto = $user->coverphoto==""?"":base_url()."/uploads/socials/coverphotos/".$user->coverphoto;
         return $user;
       }else{
         return null;
       }
    ...
    function getUserPhotos($email, $res){
      ...
      $row = $query->getRow(0);
      if($row){
        $res->photo = $res->thumbnail == ""?"":base_url()."/uploads/members/".$row->thumbnail;
        $res->coverphoto = $res->coverphoto == ""?"":base_url()."/uploads/members/".$row->coverphoto;
      }
      return $res;
    }
